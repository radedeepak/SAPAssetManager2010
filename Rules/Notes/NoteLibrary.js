        ...
        return this.noteDownload(pageClientAPI).then(note => {
            if (note && note.TextString) {
                return note.TextString.replace(/\d{4}-[01]\d-[0-3]\d [0-2]\d:[0-5]\d:[0-5]\d [A-Z]{3}/g, function(match) {
                    ...
                    let offset = libCommon.getBackendOffsetFromSystemProperty(pageClientAPI);
                    let date = new Date(new Date(dateStr).getTime() - offset * 60 * 60 * 1000);
                    return pageClientAPI.formatDatetime(date);
                });
            }
            return '-';
        });
